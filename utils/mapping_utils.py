...
def load_pose(pose_filepath, flag=True):
    if flag:
        with open(pose_filepath, "r") as f:
            line = f.readline()
            row = [float(x) for x in line.split()]
        ...
        line = pose_filepath
        row = [float(x) for x in line.split()[1:-1]]
        pos = np.array(row[:3], dtype=float).reshape((3, 1))
        quat = row[3:]
        r = R.from_quat(quat)
        rot = r.as_matrix()
...
'''
def depth2pc(depth, fov=90, intr_mat=None, min_depth=0.1, max_depth=2):
    """
    ...
    # Step3. converts the 2D pixel coordinates (p_2d) to 3D camera coordinates (pc) 
    pc = cam_mat_inv @ p_2d
    pc = pc * z     # scale up
...
def get_sim_cam_mat_with_fov(h, w, fov):
    cam_mat = np.eye(3)
    cam_mat[0, 0] = cam_mat[1, 1] = w / (2.0 * np.tan(np.deg2rad(fov / 2))) # focal length
    cam_mat[0, 2] = w / 2.0                                                 # principal point on the image coordinate
    cam_mat[1, 2] = h / 2.0                                                 # principal point on the image coordinate
    return cam_mat

'''
intrinsic matrix without field of view
'''
